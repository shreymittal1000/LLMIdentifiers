...
if [ "$game" = "base" ]; then
  ...
  do
    ...
    do
      ...
      python3 main.py "$model_0" "$model_1" "$game" "$num_rounds" "$spv" "v6.$perm" "$cues_or_cold"
      
      echo "Run #$i: $model_0 vs $model_1 -> $game with SPV: $spv and GPV: v4.$perm"
      python3 main.py "$model_0" "$model_1" "$game" "$num_rounds" "$spv" "v7.$perm" "$cues_or_cold"
      
      echo "Run #$i: $model_0 vs $model_1 -> $game with SPV: $spv and GPV: v5.$perm"
      python3 main.py "$model_0" "$model_1" "$game" "$num_rounds" "$spv" "v8.$perm" "$cues_or_cold"
    done
  done
else
  for i in {1..10}
